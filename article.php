...
elseif(isset($_REQUEST['action']) && $_REQUEST['action'] == "edit" && ($userdata['editarticle'] >= 1 || $userdata['staticarticle'] == 1) &&
	...
	if(isset($_POST['do']) && $_POST['do'] == "update" && 
		...
		if($userdata['editarticle'] == 2 || $userdata['editarticle'] == 1 && $userdata['id'] == $uidrow['uid']){
			// Anderen Autor für Artikel setzten
			$autorid_q = "";
...
<?PHP
	...
	while($row = mysql_fetch_array($list)){
		...
		<td class=\"".$class."\" title=\"".strip_tags(stripslashes(substr($row['text'],0,300)))."\" onmouseover=\"fade_element('copyid_".$row['id']."')\" onmouseout=\"fade_element('copyid_".$row['id']."')\">".stripslashes($row['titel'])." <div class=\"moo_inlinehide\" id=\"copyid_".$row['id']."\"><a href=\"".$filename."&amp;action=new".$input_section."&amp;copyid=".$row['id']."\"><img src=\"".$modulpath."images/icon_copy.gif\" alt=\"Kopieren\" title=\"Artikel zum Kopieren ausw&auml;hlen\" /></div></td>
